...
employeeRouter.get('/all',(req,res)=>{
    Employee.find({}).exec((err,documents) => {
        if(err)
            res.json({error : "Error trying to search for employees"});
        else
...
employeeRouter.get('/name/:name',(req,res)=>{
    ...
    Employee.findOne({Name : name}).exec((err,document) => {
        if(err)
            ...
            res.json({result : "Employee not found"});
    });
});
...
employeeRouter.get('/projects/:employeeId',(req,res)=>{
    ...
    Employee.findById({_id : employeeId}).populate({path: 'Project_Id'}).exec((err,document)=>{
		if(err)
			res.json({error : "Error finding employee"});
		else
...
employeeRouter.post('/new',(req,res)=>{
    ...
    employee.save(err => {
        if(err)
            res.json({error : "Error saving the new employee into db"});
        else
            res.json({message : "Successfully created employee"});
    });
...
employeeRouter.put('/modify/:employeeId',(req,res)=>{
    ...
    Employee.updateOne({_id : employeeId},
		...
		if(err)
            res.json({error : "Error updating the employee"});
        else{
...
employeeRouter.delete('/delete/:employeeId',(req,res)=>{
    ...
    Employee.findOneAndDelete({_id : employeeId}).exec(err => {
		if(err)
            res.json({error : "Error deleting the employee"});
        else{
